    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(participant => `<li>${participant} <button class="delete-btn" onclick="deleteParticipant('${name}', '${participant}')">✕</button></li>`).join('')}</ul>`
